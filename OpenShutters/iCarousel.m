...
#define MIN_TOGGLE_DURATION .9
#define MAX_TOGGLE_DURATION 0.4
#define SCROLL_DURATION 0.1
...
{   
    ...
    switch (_type)
    {
        ...
        case iCarouselTypeCoverFlow2:
        {
            CGFloat tilt = [self valueForOption:iCarouselOptionTilt withDefault:.01f];
            CGFloat spacing = [self valueForOption:iCarouselOptionSpacing withDefault:1.3];
...
-(void)depthSortViews
{
    ...
    {
        [_contentView bringSubviewToFront:(UIView *)view.superview];
    }
}
...
- (UIView *)loadViewAtIndex:(NSInteger)index withContainerView:(UIView *)containerView
{
    ...
    _contentView.superview.backgroundColor=[UIColor clearColor];
    //view.superview.layer.opacity = 0.0;
//    view.layer.cornerRadius=12.0f;
//    view.layer.masksToBounds = YES;
//  //  view.layer.cornerRadius = 15.0f; // if you like rounded corners
...
//    _contentView.layer.shadowOpacity = 0.33;
...
- (CGFloat)minScrollDistanceFromOffset:(CGFloat)fromOffset toOffset:(CGFloat)toOffset
{
    ...
    if (_wrapEnabled)
    {
        CGFloat wrappedDistance = MIN(toOffset, fromOffset) + _numberOfItems/2 - MAX(toOffset, fromOffset);
...
- (void)didTap:(UITapGestureRecognizer *)tapGesture
{
    ...
    NSInteger index = [self indexOfItemView:[self itemViewAtPoint:[tapGesture locationInView:self]]];
    if (index != NSNotFound)
    {
